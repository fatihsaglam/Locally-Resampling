locally_weighted_naive_bayes <- function(x_train, y_train, k_neighbour = 50){
...
predict_locally_weighted_naive_bayes <- function(object, newdata, type = "prob"){
  ...
  for (i in 1:n_test) {
    ...
    x_train_selected_classes <- lapply(class_names, function(m) x_train_selected[y_train_selected == m,,drop = FALSE])
    
    weights_classes <- lapply(class_names, function(m) weights[y_train_selected == m])
