locally_weighted_naive_bayes_RUS <- function(x_train, y_train, k_neighbour = 50){
...
predict_locally_weighted_naive_bayes_RUS <- function(object, newdata, type = "prob"){
  ...
  for (i in 1:n_test) {
    ...
    if (all(table(y_train_selected) > 2)) {
      dat_smote <- RUS(x_train = x_train_selected, 
