locally_weighted_naive_bayes_ROS <- function(x_train, y_train, k_neighbour = 50){
  ...
  class_pos <- class_names[class_names != class_neg]
...
predict_locally_weighted_naive_bayes_ROS <- function(object, newdata, type = "prob"){
  ...
  for (i in 1:n_test) {
    ...
    if (all(table(y_train_selected) > 2)) {
      dat_smote <- ROS(x_train = x_train_selected, 
